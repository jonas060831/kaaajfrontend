...
const sendSignInNotificationEmail = async (SignInNotificationEmail:any, signedInUserEmail: any) => {
    try {
        
        const options = {
            method: 'POST',
            headers: { 'Content-Type' : 'application/json' },
            body: JSON.stringify({ SignInNotificationEmail, signedInUserEmail }) //have to send the actualy data without the keyname
        }
